...
// ScanHSTS checks for Strict-Transport-Security (HSTS) Bypass vulnerabilities in the provided subdomains.
func ScanHSTS(subdomains []string) {
	...
	fmt.Println(banner("==================================="))
	fmt.Println(success("   Strict-Transport-Security (HSTS) Bypass   "))
	fmt.Println(banner("==================================="))

	outputDir := "./output/hsts"
	...
	resultsFile := filepath.Join(outputDir, "hsts_vulnerabilities.txt")
	...
	for _, subdomain := range subdomains {
		...
		fmt.Printf(color.YellowString("Scanning %s for HSTS vulnerabilities...\n"), baseURL)
		...
		if hstsHeader == "" {
			...
			fmt.Printf(color.RedString("Vulnerable: %s (Missing Strict-Transport-Security header)\n"), baseURL)
		} else {
			fmt.Printf(color.GreenString("Secure: %s (Strict-Transport-Security header found: %s)\n"), baseURL, hstsHeader)
		}
	}

	fmt.Println(success("HSTS scan completed. Results saved to:"), resultsFile)
}
