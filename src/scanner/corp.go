...
// ScanCORP checks for Cross-Origin-Resource-Policy (CORP) Missing vulnerabilities in the provided subdomains.
func ScanCORP(subdomains []string) {
	...
	fmt.Println(banner("==================================="))
	fmt.Println(success("   Cross-Origin-Resource-Policy (CORP) Missing   "))
	fmt.Println(banner("==================================="))

	outputDir := "./output/corp"
	...
	resultsFile := filepath.Join(outputDir, "corp_vulnerabilities.txt")
	...
	for _, subdomain := range subdomains {
		...
		fmt.Printf(color.YellowString("Scanning %s for CORP vulnerabilities...\n"), baseURL)
		...
		// Check for the Cross-Origin-Resource-Policy header
		corpHeader := resp.Header.Get("Cross-Origin-Resource-Policy")
		if corpHeader == "" {
			...
			fmt.Printf(color.RedString("Vulnerable: %s (Missing Cross-Origin-Resource-Policy header)\n"), baseURL)
		} else {
			fmt.Printf(color.GreenString("Secure: %s (CORP header found: %s)\n"), baseURL, corpHeader)
		}
	}

	fmt.Println(success("CORP scan completed. Results saved to:"), resultsFile)
}
